...
# Limit of 3 cities per tax office
for d in Cities:
    prob += (
        lpSum(depends[c][d] for c in Cities) <= 3
    )

# City can only Depends on 1 office that is less than 20km away
for c in Cities:
    prob += (
        lpSum(depends[c][d] for d in Cities if Distance[c][d] <= 20) == 1
    )
    prob += (
        lpSum(depends[c][d] for d in Cities if Distance[c][d] > 20) == 0
    )

# City can only depend on built offices
for c in Cities:
    for d in Cities:
...
# Objective function is to minimise no of tax offices
prob += (
    lpSum(build[c] for c in Cities)
)


# City must

prob.writeLP("TaxOffice.lp")
